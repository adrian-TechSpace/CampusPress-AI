...
async function verifyCategorySuggestion(user) {
  ...
  if (suggestionBody.suggestion) {
    ...
    assert.equal(
      isOpenAiQuotaLimitation(suggestionBody),
      true,
      "OpenAI category suggestion route must return a suggestion unless the known OpenAI billing limitation is active",
    );
    evidence.openAiCategorySuggestionLimitation =
      "OpenAI-reachable category suggestion remains unverified because the provider returned insufficient_quota.";
  }
...
function isOpenAiQuotaLimitation(body) {
  const error = body?.debug?.providerBody?.error;
  return body?.debug?.providerStatus === 429 && error?.code === "credit_balance_exhausted";
}
